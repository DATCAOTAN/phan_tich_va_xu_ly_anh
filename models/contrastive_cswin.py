...
def dist_collect(x):
    ...
    out_list = torch.distributed.all_gather(out_list, x)
...
class MoBY(nn.Module):
    def __init__(self,
        ...
        self.projector = MoBYMLP(in_dim=self.encoder.head.out_features, num_layers=proj_num_layers)
        self.projector_k = MoBYMLP(in_dim=self.encoder.head.out_features, num_layers=proj_num_layers)
    ...
    @torch.no_grad()
    def _momentum_update_key_encoder(self,contrast_momentum):
        """
        Momentum update of the key encoder
        """
        _contrast_momentum = 1. - (1. - self.contrast_momentum) * (np.cos(np.pi * contrast_momentum/self.total_epochs) + 1.) * 0.5
    ...
    def contrastive_queue_loss(self, q, k, queue):
    ...
    def contrastive_batch_loss(self, q, k):
        ...
        return nn.CrossEntropyLoss()(logits, labels) * (2 * self.contrast_temperature)

    def forward(self, im_1, im_2,contrast_momentum):
        ...
        with torch.no_grad():  # no gradient to keys
            self._momentum_update_key_encoder(contrast_momentum)  # update the key encoder
        ...
        # compute loss
        loss = self.contrastive_queue_loss(pred_1, proj_2_ng, self.queue2) \
            + self.contrastive_queue_loss(pred_2, proj_1_ng, self.queue1)\
