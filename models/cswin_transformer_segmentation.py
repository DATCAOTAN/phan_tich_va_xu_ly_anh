...
class LePEAttention(nn.Module):
    #dim:[96,96*2,96*4,96*8],num_heads=[4,8,16,32],reso=img_size//[4,8,16,32],split_size=[1,2,7,7]
...
class CSWinBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        img = self.norm1(x)#LN
        temp = self.qkv(img).reshape(B, H, W, 3, C).permute(0, 3, 4, 1, 2)#B, H, W, 3, C -> B, 3, C, H, W
        
        if self.branch_num == 2:
            x1 = self.attns[0](temp[:,:,:C//2,:,:])#前一半通道
            x2 = self.attns[1](temp[:,:,C//2:,:,:])#后一半通道
...
CSWin-base:
        ...
        use_chk=False,
    """
    def __init__(self, img_size=224, patch_size=4,in_chans=3,num_classes=1000, embed_dim=64, depth=[1,2,21,1], split_size = 7,
                 num_heads=[1,2,4,8], mlp_ratio=4., qkv_bias=False, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
                 drop_path_rate=0., hybrid_backbone=None, norm_layer=nn.LayerNorm, use_chk=False,
                 aux_loss=False,cross_frist=False,cross=False):
        super().__init__()
    ...
    def forward_features(self, x):
        ...
        B, C, H, W = x.size()
        x = x.reshape(B, C, -1).transpose(-1,-2).contiguous()#B,N,C  ->(N=HW)
        ...
        if self.cross and self.cross_frist:
            out.append(self.norm1(x))
        else:
    ...
    def forward(self, x):
        x,result = self.forward_features(x)
        if self.aux_loss:
